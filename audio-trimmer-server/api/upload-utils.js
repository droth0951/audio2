...
class AudioFileProcessor {
   ...
   * @param {string} audioUrl - The podcast audio URL
  ...
  async downloadAndExtractSegment(audioUrl, startSeconds, endSeconds) {
    ...
    console.log('  Segment duration:', `${endSeconds - startSeconds}s`);
    ...
    try {
      ...
      console.log(`✅ Downloaded ${Math.round(stats.size / 1024 / 1024 * 100) / 100}MB to:`, fullAudioPath);

      // Step 2: Extract the specific segment using FFmpeg
   ...
   * @returns {Promise<string>} - AssemblyAI upload URL
   */
   ...
   * @param {string} audioUrl - Source audio URL
   * @param {number} startSeconds - Start time in seconds
   * @param {number} endSeconds - End time in seconds  
   * @param {string} apiKey - AssemblyAI API key
   * @returns {Promise<string>} - AssemblyAI upload URL
